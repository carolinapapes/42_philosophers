...
/*   Updated: 2024/07/11 17:36:41 by capapes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (print_action((get_time() - program->time_start) \
	* 0.001, index, str));
}

static inline int	set_philo_e(t_philo *philo)
{
	*(&(philo->err)) = 1;
	return (1);
}
...
int	philo__write(t_program *program, t_philo *philo, char *str)
{
	return ((\
		...
		(program->philos_end && set_philo_e(philo)) || \
		(print_action__now(program, philo->index, str) == -1 \
		&& exit_(philo, CLEAN__MX_PUT)) || \
