...
/*   Updated: 2024/06/30 00:01:12 by capapes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fork__get(t_philosopher *philo)
{
	...
	ph_philo__write(philo, "Takes rf");
	pthread_mutex_lock(philo->left_fork);
	ph_philo__write(philo, "Takes lf");
}

static void	fork__drop(t_philosopher *philo)
{
	ph_philo__write(philo, "Drops rf");
	pthread_mutex_unlock(&philo->right_fork);
	ph_philo__write(philo, "Drops lf");
	pthread_mutex_unlock(philo->left_fork);
}

static void	meal__get(t_philosopher *philo)
{
	ph_philo__write(philo, "Eats");
